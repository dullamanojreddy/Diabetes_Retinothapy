...
- **Phase 11 (External Validation and Benchmarking)**: Complete. Implemented `evaluate_external.py` and `export_predictions.py` calculating Quadratic Weighted Kappa (QWK), Sensitivity, Specificity, Macro-F1, and Confusion Matrices. Documented clinical validation protocols and separation from published literature in `docs/VALIDATION.md` and latency/throughput benchmarks in `docs/BENCHMARKS.md`.
- **Phase 12 (MATLAB Research Evidence — Excluding Simulink)**: Complete. Implemented classical computer vision and statistical analysis scripts in `matlab/`: image quality (`assess_quality.m`), conservative enhancement (`enhance_fundus.m`), morphological vessel segmentation (`segment_vessels.m`), and predictions import & distribution analysis (`import_predictions.m`) with full user guide in `matlab/README.md`.
- **75/75 pytest tests passing** (70 baseline regression tests preserved + 5 Phase 11 tests).
